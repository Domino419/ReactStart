...
## 10.1 프로젝트 준비하기[저는 쿠팡 직원입니다.blind[415] 아르바이트 19-35.url](../Users/bluem/AppData/Local/Temp/%EC%A0%80%EB%8A%94%20%EC%BF%A0%ED%8C%A1%20%EC%A7%81%EC%9B%90%EC%9E%85%EB%8B%88%EB%8B%A4.blind%5B415%5D%20%EC%95%84%EB%A5%B4%EB%B0%94%EC%9D%B4%ED%8A%B8%2019-35.url)
...
- React DwvTools 의 Profiler 탭에서 녹화 버튼을 통해서 리랜더링에 소요된 시간 확인 가능
- v17 부터만 이용가능함. 
## 11.3 느려지는 원인 분석
- 리렌더링이 발생하는 상황 : 자신이 전달받은 props가 변경될 떄, 자신의 state가 바뀔 때, 부모 컴포넌트가 리랜더링될 때, forceUpdate 함수가 실행될 때)
- 리랜더링이 불필요한 경우에는 리랜더링 방지 처리를 해야함.
...
- todos 배열이 업데이트 되더라도 onRemove와 onToggle 함수가 바뀌게 되는 걸 방지 하기.
### 11.5.1 useState의 함수형 업데이트
### 11.5.2 useReducer 사용하기
## 11.6 불변성의 중요성
- 기존의 값을 직접 수정하지 않으면서 새로운 값을 만들어 내는 것 = 불변성을 지킨다.
- 객체 안에 있는 객체인 경우 불변성을 지키면서 새 값을 할당해야 하므로 immer라는 라이브러리를 통하여 깊은 복사 가능.
## 11.7 TodoList 컴포넌트 최적화하기
- 리스트 관련 컴포넌트를 작성할 때에는 리스트 아이템과 리스트, 두가지 컴포넌트에 대해서 최적화 해주기.
